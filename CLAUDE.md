...
**⚠️ Limitations réseau connues (sessions Claude Code sur le web) :**
- `dl.google.com` (dépôt Maven Google, déclaré dans `settings.gradle.kts` via `google { mavenContent { includeGroupAndSubgroups("androidx") ... } } }`) était bloqué par la politique d'egress de l'environnement — **résolu** (allowlist mise à jour) : `./gradlew jvmTest` fonctionne à nouveau en session web.
- Le téléchargement de binaires tiers hébergés sur GitHub Releases (la distribution Gradle elle-même via `services.gradle.org` → `github.com/gradle/gradle-distributions`, et Yarn via `github.com/yarnpkg/yarn`) reste bloqué : les sessions Claude Code web restreignent l'accès `github.com` aux dépôts explicitement ajoutés à la session (mécanisme distinct de l'allowlist réseau, voir "GitHub access to this repository is not enabled for this session"). Conséquence : `./gradlew` ne peut pas bootstrapper la distribution Gradle pinnée (d'où le fallback vers `gradle` système dans `.claude/hooks/session-start.sh` et `.githooks/pre-push`), et `kotlinNodeJsSetup`/`kotlinNpmInstall` (donc `jsBrowserDevelopmentRun`/`jsBrowserProductionWebpack`) échouent tant que Yarn n'est pas téléchargeable. Pas un problème de config projet.
