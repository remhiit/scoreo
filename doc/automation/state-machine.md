...
| 4 | Issue | `automation:ready` | `issues.labeled(automation:ready)` | R2 (`implement-task`) | `automation:in-progress`; branch, code, tests, doc, PR opened with `Closes #N`; `automation:enabled` posed only if risk stays Faible | Auto |
| 5 | Issue | `automation:ready` | `issues.labeled(automation:ready)` fires but the issue is no longer actionable (already claimed, closed) | R2 | `automation:ready` unchanged — R2 does nothing else, never picks a different issue | Auto (guard) |
| 6 | Issue | `automation:ready` | Spec turns out ambiguous/incomplete once R2 reads it fully | R2 | `automation:in-progress` → `automation:needs-human`; comment naming what's missing | Human (escalated) |
...
- **Claiming the label isn't enough to rule out two reviews on one SHA.** A
  ...
  re-poses the recorded verdict label instead of reviewing again.
