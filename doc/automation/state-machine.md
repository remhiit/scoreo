...
| `risk-controls` (`risk-controls.yml`, `scripts/risk-controls.mjs`) | Action | `pull_request` `opened`/`synchronize`/`reopened`/`labeled` — its own workflow, not a job in `ci.yml`, precisely so `labeled` re-runs it (`ci.yml`'s default `pull_request:` trigger only covers `opened`/`synchronize`/`reopened`, and `automation:enabled` is posed via a separate `labeled` event, never in the same event that runs `ci.yml`) | Refuses `automation:enabled` on a PR whose linked issue (`Closes #N`) reads risk `high`, or whose risk section is missing/unreadable (normalized to `high`) — fails the check naming the issue and the violated rule (issue #479); reuses `extractClosedIssueNumbers` (`scripts/close-linked-issues.mjs`) and `extractRiskLevel` (`scripts/routing-dry-run.mjs`) rather than reparsing either |
