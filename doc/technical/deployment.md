...
| `design-tokens` | `node scripts/check-design-tokens.mjs` — fails if a raw px/duration/easing value in `apps/scoreo/public/css/*.css` or `packages/module-tori-valley/src/styles.css` exactly matches a design token (see `doc/technical/architecture.md` §Design tokens) | Yes |
