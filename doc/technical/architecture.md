...
| `sw.js` | Hand-written service worker — network-first for navigation, cache-first for everything else |
...
`ThemeProvider`/`useTheme()` (`src/ui/theme/ThemeContext.tsx`) is a React Context so the burger menu's theme picker and the rest of the app share the same live flavor/accent state. The pure logic (`readInitialFlavor`/`readInitialAccent`/`applyTheme`) lives in `src/ui/theme/themeManager.ts` and is directly unit-tested. CSS token files (`colors-{latte,frappe,macchiato,mocha}.css`, `semantic.css`, etc.) live in `public/css/tokens/`, native `@import` chain.
