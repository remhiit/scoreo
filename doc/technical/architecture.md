...
- **Dictionaries**: flat-ish nested JSON, one file per language — `src/i18n/locales/en.json` (reference/fallback, mirrors the text that used to be hardcoded in JSX) and `locales/fr.json` (full translation). Both share the same key shape, grouped by a `common` namespace (`cancel`/`close`/`delete`/`confirm` — reused across several modals) and one namespace per screen/component (`menu`, `languagePicker`, `home`, `gametype`, `history`, `scoreDetail`). `resolveJsonModule` is enabled in `tsconfig.app.json` so these import as typed objects.
