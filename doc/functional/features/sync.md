...
4. The coordinator is started/stopped by the `useAutoSync` hook (`apps/scoreo/src/ui/sync/useAutoSync.ts`), called once from `AppShell`. It's a no-op when `services.syncUseCase` is undefined (no `VITE_GOOGLE_CLIENT_ID`).
