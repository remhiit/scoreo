...
| Import: v1.0 file, v1.1 file, duplicate skip, mismatched-details failure | ✅ Both format versions import correctly; re-importing the same file skips already-imported ids; a match whose `details` don't sum to the final score is reported as failed with its id; a match whose `details` do sum correctly stores them as `rounds`, shown in History like a match scored in Scoreo |
