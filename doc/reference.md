...
| `LudoTable<T>` | `columns: LudoColumn<T>[]` (`header`, `align?`, `render: (row: T) => ReactNode`), `rows: T[]`, `rowKey: (row: T) => string`, `footer?`, `striped?` (default `true`), `dense?` | Scoreboard grid. `RoundHistoryList`'s per-round cards (wrapping cells, per-round delete, free-text editable scores) don't fit this shape and stay hand-rolled with `.hist-*` classes instead. |
