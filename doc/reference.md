...
| `MergePlayersUseCase` | `preview(duplicateId, keptId)` | `MergePlayersPreview` (`{ affectedMatches, conflictingMatches }`) — counts only, no validation |
| `MergePlayersUseCase` | `invoke(duplicateId, keptId)` | `void` — rewrites every match reference to the duplicate (`playerScores`, `secondaryPlayerScores`, `rounds`, `manualWinners`) onto the kept player via one `saveAll`, clears a `MatchDraft` naming the duplicate, reactivates a soft-deleted target when the duplicate was active, then `hardDelete`s the duplicate. Throws `NotFoundError` for an unknown id, `ValidationError` for a self-merge or when any match involves both players |
| `MergeGameTypesUseCase` | `preview(duplicateId, keptId)` | `MergeGameTypesPreview` (`{ affectedMatches, rulesDiffer }`) — `rulesDiffer` compares `winCondition`/`tieBreakRule` (+ `tieBreakCondition` only under `SECONDARY_SCORE`); a warning, never a blocker |
| `MergeGameTypesUseCase` | `invoke(duplicateId, keptId)` | `void` — moves every match's `gameTypeId` onto the kept game type via one `saveAll`, clears a `MatchDraft` naming the duplicate, unarchives the target when the duplicate was active, then `hardDelete`s the duplicate. Throws `NotFoundError` for an unknown id, `ValidationError` for a self-merge |
...
| `MergeSelectModal` | `open`, `title`, `body`, `duplicateLabel`, `keptLabel`, `placeholder`, `options: MergeSelectOption[]`, `duplicateId`, `keptId`, `onSelectDuplicate`, `onSelectKept`, `summary?`, `warning?`, `blocked?`, `confirmText`, `error`, `onClose`, `onConfirm` | Two-picker merge dialog shared by `MergePlayersModal` (Home) and `MergeGameTypesModal` (Games): pick the duplicate to absorb, pick the one to keep, read the summary, confirm. The duplicate is filtered out of the "keep" list, so the two can never name the same entity; confirmation stays disabled until both are picked, and `blocked` keeps it disabled while also painting `warning` in `--color-danger` (`.merge-warning--blocking`) instead of `--color-warning`. Option labels arrive pre-decorated by the caller ("(deleted)" / "(archived)"). |
...
78 test files, 993 tests, all colocated `*.test.ts(x)` next to the file they cover, running under Vitest + `jsdom` (no real browser needed for any of them, including the Google Drive/OAuth and theme tests that historically required one).
...
- `e2e/merge-players.spec.ts` — the duplicate-player story end to end: creates three players, plays a match under the duplicate, merges it into the player kept from Home's "Merge" dialog, then checks the duplicate is gone from Home and its win now shows on the kept player's Stats row.
