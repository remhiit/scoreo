...
`src/App.tsx` (`AppShell`, dispatch by `useHashRouter().current`) replaces `App.kt`'s root Composable — wraps content in `ServicesProvider`/`ThemeProvider` (which `Main.kt` didn't need since Compose has no context-provider pattern). Screen bodies are placeholders until their Phase F ticket lands, except `Stats` (TS-050), `History` (TS-051), `Games` (TS-052), `Import` (TS-053), `Sync` (TS-054), and `Home` (TS-055) which are real (only `ScoreDetail` remains a placeholder); the header, contextual back button, and burger menu are the real, final logic:
