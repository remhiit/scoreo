...
| Sync | `src/ui/sync/syncReducer.ts` | `SyncAction` | `loginStarted`, `loginFailed`, `connected`, `synced`, `conflictDetected`, `syncFailed`, `loggedOut`, `resolvingConflict`, `conflictResolved`, `conflictResolveFailed`, `dismissError` | `src/ui/sync/syncTypes.ts` (`SyncState`, `phase: SyncPhase`) |
