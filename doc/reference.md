...
| `LudoTable<T>` | `columns: List<LudoColumn<T>>` (`header: String`, `align: String? = null`, `render: @Composable (T) -> Unit`), `rows: List<T>`, `footer: (@Composable () -> Unit)? = null`, `striped: Boolean = true`, `dense: Boolean = false` | Ludo scoreboard grid (`.ludo-table*`) — real `<thead>`/`<tbody>`/`<tfoot>` (unlike `ScoreDetailScreen`'s hand-rolled table, which puts `Tr`s directly under `Table` — needed here so the `tbody`/`tfoot`-scoped CSS selectors actually match). `footer` renders inside its own `Tr`; the caller supplies the `Td` cells. Not yet evaluated against `ScoreDetailScreen`'s editable-cells-with-actions-column case — see `.task/P2/03-*`. |
| `LudoModal` | `open: Boolean`, `title: String? = null`, `onClose: (() -> Unit)? = null`, `footer: (@Composable () -> Unit)? = null`, `content: @Composable () -> Unit` | Ludo centered dialog (`.ludo-modal*`) — scrim click / Escape both close via `onClose`. Simpler than the design system's vanilla-JS version: `if (open) { ... }` is enough, no persistent open()/close() controller needed since Compose HTML diffs the DOM itself. Not yet wired into any confirmation flow — see `.task/P2/*` for migrating off `.modal-overlay`/`.modal-content` (including the theme picker added in the P0-03 ticket, which predates this composable and still uses the legacy pattern). |
...
Key classes: `.btn-icon`, `.btn-icon--danger`, `.btn-danger`, `.btn-danger-filled` (still used by History/ScoreDetail/GameType — pending their own P2 tickets), `.modal-body`, `.detail-row`, `.detail-label`, `.detail-value`, `.badge-warn`, `.tie-break-info`, `.splash`, `.splash-content`, `.spinner`, `.onboarding-guide`, `.fab-position`, `.list-container`, `.list-container--spaced`, `.list-item-row`, `.list-item-label`, `.list-item-label--selectable`, `.list-item-label--selected`, `.list-item-name`, `.list-item-subtitle`, `.list-item-actions`, `.list-item-select-picto`.

HomeScreen (P2-02) now uses `LudoButton`/`LudoTextInput`/`LudoModal` throughout — `.fab`/`.fab-disabled`, `.btn-add`, `.player-info`/`.player-rename-*`/`.btn-edit`/`.btn-sm` (dead code found and removed — no Kotlin referenced them), and the local `.btn-primary`/`.btn-secondary` duplicate in `home.css` are gone.
